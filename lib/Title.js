	...
	fetch() {
		...
		return new Promise(function (resolve, reject) {
			...
			self.mal.get$(self.path, {}).catch(reject).then(function ($) {
				...
				let altTitles = $('h2').filter(function () {
					return $(this).text().trim() === 'Alternative Titles';
				})
				if (altTitles) {
					self.english = altTitles.siblings('div').filter(function () {
						return $(this).children().first().text().trim().startsWith('English:')
					}).first().text().trim().replace('English: ', '');
					self.synonyms = altTitles.siblings('div').filter(function () {
						return $(this).children().first().text().trim().startsWith('Synonyms:')
					}).first().text().trim().replace('Synonyms: ', '');
				} else {
					self.english = ''
					self.synonyms = ''
				}
